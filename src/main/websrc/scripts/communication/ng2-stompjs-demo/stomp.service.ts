...
export class STOMPService {
	...
	/** Constructor */
	public constructor() {
		this.messages = new Subject<Frame>();
	...
	public configure(config?: StompConfig): void {
		...
		// Attempt connection, passing in a action
		this.client = Stomp.client(`${scheme}://${this.config.host}/CTUConference/conversation2`);
	...
	/** Send a message to selected topic */
	public send(destination: string, body?: any, headers?: {}) {
	...
	public on_error = (error: string | Frame) => {
		if (typeof error === 'object') {
			error = (<Frame>error).body;
		}

		console.error(`Error: ${error}`);

		// Check for dropped connection and try reconnecting
		if ((<string>error).indexOf('Lost connection') !== -1) {
